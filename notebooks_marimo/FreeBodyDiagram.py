...
def _(mo):
    ...
    \\
    \sum \mathbf{M} = 0
    \end{array}$That is, the **vectorial** sum of all forces and moments of force acting on the body must be zero hence the body is not moving or rotating.<br>
...
def _(mo):
    ...
    > *The moment of a force about a point is equal to the sum of moments of the components of the force about the same point.*<br>
...
def _(mo):
    ...
    Our interest is to draw the FBD for the ball:<br>
    <br>
    ...
    W is the weight force due to the gravitational forces which act on all particles of the ball but the effects of the weight force on each particle of the ball are the same if we draw only one force (the ball weight) acting on the center of gravity of the ball.<br>
    N is the normal force, it is perpendicular to the surface of contact and prevents the ball from penetrating the surface.

    The vector representing a force must be drawn with its origin at the point of application of the force. However, for sake of clarity one might draw the contect forces outside the body with the tip of the vector at the contact region (note that in the figure above, we had to draw small vectors to avoid the superposition of them).<br>
    ...
    From the FBD, one can now derive the equilibrium equations:<br>
    ...
    This very simple case already illustrates a common problem when drawing a FBD and deriving the equations:<br>
    ...
    If in the reference system we chose, the upward direction is positive, the value of$\mathbf{W}$should be negative.<br>
    So, if we write the equation as$\mathbf{N}+\mathbf{W}=0$, it's because we are representing the vectors and the value of$\mathbf{W}$is either equal to$m\mathbf{g}$(with$g=-10 m/s^2$) or$-m\mathbf{g}$(with$g=10 m/s^2$).<br>
    ...
    The best option is to draw and write in the equations in vectorial form, without their signals and let the signals appear when the numerical values for these quantities are inputted.<br>
...
def _(mo):
    ...
    Our interest is to draw the FBD for a person standing still:<br>
    <br>
...
def _(mo):
    ...
    Our interest is to draw the FBD for both boxes:<br>
    <br>
    ...
    At the end of the first example it's written: "The best option is to draw and write in the equations, the vectors, without their signals and let the signals appear when the numerical values for these quantities are inputted."<br>
...
def _(mo):
    ...
    Now we have a segment fixed to a base by a joint and we want to draw the FBD for the segment:<br>
    <br>
    ...
    On any joint one can have a force and a moment of force (but a joint with a free axis of rotation, offering no resistance to rotation, generates no moment of force, only joint forces).<br>
    For this example, we guessed the most general case, a joint with a force and a moment of force, but we may find later that one of these quantities is zero, this is ok.<br>
    ...
    To find the direction of the moment of force might be difficult sometimes. If we had numbers for all these variables, we could simply do the mathematical operations and let the results indicate the direction.<br>
    For example, suppose the center of gravity is at the origin and that the bar has mass 1 kg and length 1 m.<br>
...
def _(mo):
    mo.md(r"""
    The moment of force$\mathbf{M}$is positive (counterclockwise direction) to balance the negative moment of force due to the gravitational force.<br>
...
def _(mo):
    ...
    Now we have two segments and two joints and we want to draw the FBD for each segment:<br>
    <br>
    ...
    This problem could have been solved calculating the moments of force around each joint instead of around the center of gravity; maybe it would have been simpler, but it would give the same results.<br>
...
def _(mo):
    ...
    The formalism above can be extended for an accelerated rigid body where the following conditions must be satisfied:$\sum \mathbf{F} = m\mathbf{a_{cm}}$$\sum \mathbf{M_O} = \frac{d\mathbf{L_O}}{dt}$Where$O$is a reference point from which the movement (translation and rotation) of the rigid body is described ($O$is an arbitrary position, it can be in any place in space).<br>
...
def _(mo):
    ...
    <figure><img src="https://github.com/BMClab/BMC/blob/master/images/reactionboard.png?raw=1" alt="Reaction board"/><figcaption><center><i>Figure. Illustration of the reaction board device for the estimation of the center of mass position on a body.</i></center></figcaption></figure>
     ...
     a) Derive the equation to determine the center of mass position considering the parameters shown in the figure above.<br>
    ...
    <figure><img src="https://github.com/BMClab/BMC/blob/master/images/arm.png?raw=1" width=400 alt="lever arm"/><figcaption><center><i>Figure. A person holding a dumbbell and some of the physical quantities related to the mechanics of the task.</i></center></figcaption></figure>
     ...
     a) Determine the value of the elbow flexion force.<br>
    ...
    6. Consider the system in Example 4, but now the segment (with$1m$length and$4kg$mass) is attached to the base by a joint that allows free rotation around the$\mathbf{z}$axis. At the instant shown in Example 4 (at the horizontal), the segment has an angular velocity equals to$-5 rad/s$. Consider$g=-10 m/s^2$. For this instant, determine:<br>
     a) The angular acceleration<br>
     b) The force at the joint<br>
