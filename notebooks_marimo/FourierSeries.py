...
def _(mo):
    mo.md(r"""
    In the XIX century, the French mathematician [Jean-Baptiste Joseph Fourier](http://en.wikipedia.org/wiki/Joseph_Fourier) made the discovery that any (in fact, almost any) complicated but periodic function could be represented as a sum of sines and cosines with varying amplitudes and frequencies, which are known as the [Fourier series](http://en.wikipedia.org/wiki/Fourier_series), in his honor.<br>
    Based on the Fourier series, he also proposed a mathematical transformation to transform functions between time (or spatial) domain and frequency domain, known as the [Fourier transform](http://en.wikipedia.org/wiki/Fourier_transform).<br>
    ...
    Let's look at some basic concepts about sinusoids and then we will look at the Fourier series.<br>
...
def _(mo):
    ...
    There is an interesting relationship between the sine, cosine, and exponential functions and complex numbers, known as the [Euler's formula](http://en.wikipedia.org/wiki/Euler%27s_formula):$e^{i\theta} = \cos \theta + i \sin \theta$Where$e$is the <a href="http://en.wikipedia.org/wiki/E_(mathematical_constant)">Napier's constant or Euler's number</a> ($e=2.71828...$, the limit of$(1 + 1/n)^n$as$n$approaches infinity, and it's also the inverse of the [natural logarithm](http://en.wikipedia.org/wiki/Natural_logarithm) function:$ln(e)=1$) and$i$is the [imaginary unit](http://en.wikipedia.org/wiki/Imaginary_number) ($\sqrt{-1}=i$).<br>
...
def _(mo):
    ...
    Let's deduce how to calculate the Fourier coefficients$a_0$,$a_k$, and$b_k$.<br>
...
def _(mo):
    ...
    The$a_k$and$b_k$coefficients are the amplitudes of the sine and cosine terms, each with frequency$k/T$, of the Fourier series.<br>
...
def _(mo):
    ...
    \end{array} \right.$If you compare with the previous calculation you will notice some differences.<br>
    First, now the expression is multiplied by 2 instead of by 4. But remember that now$k$goes from$-\infty$to$\infty$, i.e., we will now add terms for$k=\pm1,\pm2,\pm3,\dots$, meaning that the expression above will be counted twice for each$|k|$.<br>
