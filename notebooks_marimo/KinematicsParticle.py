...
def _(mo):
    ...
    Some mechanical quantities in Kinematics (position and its derivatives) are represented as vectors and others, such as time and distance, are scalars.<br>
...
def _(mo):
    ...
    Consider a point in the three-dimensional Euclidean space described in a Cartesian coordinate system (see the notebook [Frame of reference](http://nbviewer.jupyter.org/github/demotu/BMC/blob/master/notebooks/ReferenceFrame.ipynb) for an introduction on coordinate systems in Mechanics and Biomechanics):<br>
    <br>
...
def _(mo):
    ...
    We can use [Sympy](http://www.sympy.org/en/index.html), a Python library for symbolic mathematics, to deduce the expressions for the cases of motion of a particle we just visualized.<br>
...
def _(mo):
    ...
    A technical report with the kinematic data for the 100-m world record by Usain Bolt can be downloaded from the [website for Research Projects](http://www.iaaf.org/development/research) from the International Association of Athletics Federations.<br>
    [Here is a direct link for that report](http://www.iaaf.org/download/download?filename=76ade5f9-75a0-4fda-b9bf-1b30be6f60d2.pdf&urlSlug=1-biomechanics-report-wc-berlin-2009-sprint). In particular, the following table shows the data for the three medalists in that race:<br>
    <br>
...
def _(mo):
    ...
    2. Consider the data for the three medalists of the 100-m dash in Berlin, 2009, shown previously.<br>
       ...
       c. Plot the graphs velocity and acceleration versus partial distance (every 20m).<br>
    ...
    3. The article "Biomechanical Analysis of the Sprint and Hurdles Events at the 2009 IAAF World Championships in Athletics" by Graubner and Nixdorf lists the 10-m split times for the three medalists of the 100-m dash in Berlin, 2009 and is shown below.<br>
    ...
    5. A body attached to a spring has its position (in cm) described by the equation$x(t) = 2\sin(4\pi t + \pi/4)$.<br>
       a. Calculate the equation for the body velocity and acceleration.<br>
    ...
    6. The rectilinear motion of a particle is given by$x(t) = -12t^3 + 15t^2 + 5t + 2$[s; m]. Calculate:<br>
       a. Velocity and acceleration of the particle as a function of time. Solution:$v(t)=36t^2+30t+5$[s; m/s],$a(t)=-72t+30$[s; m/s2].<br>
       b. Total distance traveled by the particle in the interval$0 \leq t \leq 4$s. Solution:$\Delta_{0-4}=524.02$m.<br>
       c. Maximum value of the velocity module reached by the particle in the interval$0 \leq t \leq 4$s. Solution:$\lVert v_{max} \rVert=451.0$m/s.<br>
