...
def _(mo):
    ...
    "Git has three main states that your files can reside in: committed, modified, and staged.<br>
    Committed means that the data is safely stored in your local database. Modified means that you have changed the file but have not committed it to your database yet. Staged means that you have marked a modified file in its current version to go into your next commit snapshot.<br>
...
def _(mo):
    ...
    You can also install a graphical user interface (GUI) for Git: [GUI Clients](http://git-scm.com/downloads/guis).<br>
    ...
    After you installed Git, you can check its version using a terminal window or the command `!` in the IPython Notebook to access the system shell.<br>
...
def _(mo):
    ...
    After installation, we need to configure Git (this is only needed once and it will be used when we commit changes to the repository).<br>
...
def _(mo):
    mo.md(r"""
    You would do that in case you want starting to track an existing local project in Git.<br>
...
def _(mo):
    mo.md(r"""
    The local repository will be created inside your current directory.<br>
    You can change the current directory to clone from there and there is no need to create a folder with the name of the repo you are cloning; Git will do that for you.<br>
...
def _(mo):
    ...
    You can propose changes (add it to the Index) using the command `add`.<br>
...
def _(mo):
    mo.md(r"""
    The command `add` is a multipurpose command; allows to track files, stage files, and mark merge conflicted files as resolved.<br>
...
def _(mo):
    mo.md(r"""
    You should do that in a regular terminal window because you may be prompted to enter your username and password for your GitHub account in case you didn't store the username and password in your OS (if you are using `HTTPS`).<br>
...
def _(mo):
    ...
    These two last commands tries to auto-merge changes.<br>
    This might not be possible because of conflicts between the different branches.<br>
    If that is the case, we will have to merge those conflicts manually by editing the files shown by git.<br>
    After changing, we need to mark them as merged with `git add <filename>` or `git add .`.<br>
