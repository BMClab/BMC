...
def _(mo):
    ...
    > Renato Naville Watanabe, Marcos Duarte<br>
...
def _(mo):
    ...
    Let's use Sympy, a symbolic library in Python, to solve this integral.<br>
...
def _(mo):
    ...
    \end{array}$where$\vec{r}_{cm_i}$is  the center of mass of the body$i$,$m_{Bi}$is the mass of the body$i$and$m_T$is the total mass of all bodies.<br>
...
def _(mo):
    ...
    Center of gravity of a body is the point where the moment of force caused by the gravitational force in the whole body relative to this point is zero.<br>
...
def _(mo):
    ...
    \end{array}$The terms$\int\limits_B x_{/cm}\,\mathrm d m$and$\int\limits_B y_{/cm}\,\mathrm d m$are equal to zero because of the definition of center of mass. The term$d$is the distance between the two axes.<br>
...
def _(mo):
    ...
    Consider a cylinder with constant density shown in the figure below and compute the moment of inertia around the three axes relative to the center of mass.<br>
...
def _(mo):
    ...
    **The product of inertia terms**
    ...
    By inspecting the problem, we know that because of symmetry of the axes w.r.t. the cylinder, the product of inertia terms of the matrix of inertia should be all zero. Let's confirm that.<br>
...
def _(mo):
    ...
    One can see that the area moment of inertia has no mass term; it has dimension$[L]^4$and SI unit$m^4$.<br>
