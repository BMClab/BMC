...
def _(mo):
    ...
    First, let's study the case of a system composed by one planar hinge joint and one link, which technically it's not a chain but it will be useful to review (or introduce) key concepts.<br>
    <br>
...
def _(mo):
    mo.md(r"""
    Using the matrix formalism will simplify things, but we will loose some of the Sympy methods for vectors (for instance, the variable `r_p` has a method `magnitude` and the variable `r` does not.<br>
...
def _(mo):
    mo.md(r"""
    Where we used the [Newton's notation](http://en.wikipedia.org/wiki/Notation_for_differentiation) for differentiation.<br>
    Note that <span class="notranslate">$\dot{\theta}$</span> represents the unknown angular velocity of the joint; this is why the derivative of <span class="notranslate">$\theta$</span> is not explicitly solved.<br>
    The magnitude or [Euclidian norm](http://en.wikipedia.org/wiki/Vector_norm) of the vector <span class="notranslate">$\overrightarrow{\mathbf{v}}$</span> is:<br>
    <br>
...
def _(mo):
    ...
    The acceleration of the endpoint position can be given by the second-order derivative of the position or by the first-order derivative of the velocity.<br>
...
def _(mo):
    mo.md(r"""
    This means that there will be a linear acceleration even if the angular acceleration is zero because although the magnitude of the linear velocity is constant in this case, its direction varies (due to the centripetal acceleration).<br>
    <br>
...
def _(mo):
    ...
    Let's plot some simulated data to have an idea of the one-link kinematics.<br>
...
def _(mo):
    ...
    We now will look at the case of a planar kinematic chain with two links, as shown below. The deduction will be similar to the case with one link we just saw.<br>
    <br>
...
def _(mo):
    ...
    Note that <span class="notranslate">$\theta_2$</span> is a joint angle (referred as measured in the **joint space**); the angle of the segment 2 with respect to the horizontal is <span class="notranslate">$\theta_1+\theta_2$</span> and is referred as an angle in the **segment space**.<br>
...
def _(mo):
    ...
    Using the [cosine rule](http://en.wikipedia.org/wiki/Law_of_cosines), in terms of the endpoint position, the angle <span class="notranslate">$\theta_2$</span> is:<br>
    <br>
    ...
    To find the angle <span class="notranslate">$\theta_1$</span>, if we now look at the triangle in red in the figure below, its angle <span class="notranslate">$\phi$</span> is:<br>
    <br>
    <span class="notranslate">$\phi = \arctan\left(\frac{\ell_2 \sin(\theta_2)}{\ell_1 + \ell_2 \cos(\theta_2)}\right)$</span>

    And the angle of its hypotenuse with the horizontal is:<br>
    <br>
    <span class="notranslate">$\theta_1 + \phi = \arctan\left(\frac{y_P}{x_P}\right)$</span>

    Then, the angle <span class="notranslate">$\theta_1$</span> is:<br>
    <br>
    ...
    Note that there are two possible sets of <span class="notranslate">$(\theta_1, \theta_2)$</span> angles for the same <span class="notranslate">$(x_P, y_P)$</span> coordinate that satisfy the equations above. The figure below shows in orange another possible configuration of the kinematic chain with the same endpoint coordinate. The other solution is <span class="notranslate">$\theta_2'=2\pi - \theta_2$</span>, but <span class="notranslate">$\sin(\theta_2')=-sin(\theta_{2})$</span> and then the <span class="notranslate">$arctan()$</span> term in the last equation becomes negative.<br>
    Even for a simple two-link chain we already have a problem of redundancy, there is more than one joint configuration for the same endpoint position; this will be much more problematic for chains with more links (more degrees of freedom).<br>
    <br>
...
def _(mo):
    mo.md(r"""
    Let's now deduce the Jacobian for this planar two-link chain:<br>
    <br>
    <span class="notranslate">$\mathbf{J} =
    \large
    \begin{bmatrix}
...
def _(mo):
    mo.md(r"""
    Using the Jacobian, the linear velocity of the endpoint is:<br>
    <br>
...
def _(mo):
    mo.md(r"""
    Once again, the linear acceleration of the endpoint is given by the derivative of the product between the Jacobian and the angular velocity:<br>
    <br>
...
def _(mo):
    ...
    Let's plot some simulated data to have an idea of the two-link kinematics.<br>
...
def _(mo):
    ...
    1. Calculate the Jacobian matrix for the following function:<br>
    <span class="notranslate">$f(x, y) = \begin{bmatrix}
    x^2 y \\
    5 x + \sin y \end{bmatrix}$</span>

    2. For the two-link chain, calculate and interpret the Jacobian and the expressions for the position, velocity, and acceleration of the endpoint for the following cases:<br>
     a) When the first joint (the joint at the base) is fixed at <span class="notranslate">$0^o$</span>.<br>
    ...
    <br>
    Solution for$\overrightarrow{v}$:<br>
