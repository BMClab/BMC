...
def _(mo):
    ...
    **Optimization is the process of finding the best value from possible alternatives with regards to a certain criteria** ([Wikipedia](http://en.wikipedia.org/wiki/Mathematical_optimization)).
    ...
    Typically, such best value is the value that maximizes or minimizes the criteria. In this context, to solve a (mathematical) optimization problem is to find the maximum or minimum (a.k.a., a stationary point) of a function (and we can use maximum or minimum interchangeably because the maximum of a function is the minimum of the negative of that function).<br>
    ...
    The optimization problem is the calculation of the minimum or maximum values of an objective function over a set of **unknown** possible values of the design variables.<br>
    Even in case of a finite number of possible values of the objective function and design variables (e.g., after discretization and a manual or a grid search), in general the evaluation of the objective function is computationally expensive and should be avoided.<br>
...
def _(mo):
    ...
    We want to make a box from a square cardboard with side$a$such that its volume should be maximum.<br>
...
def _(mo):
    ...
    \\
    V(b) = a^2b - 4ab^2 + 4b^3
    \end{array}$In the context of optimization:<br>
    **The expression for$V$is the cost function,$b$is the design variable, and the constraint is that feasible values of$b$are in the interval$]0, \dfrac{a}{2}[$, i.e.,$b>0$and$b<\dfrac{a}{2}$.**
    ...
    \end{array}$We have to find the values for$b$where the first derivative of$V$is zero (the extrema) and then use the expression for the second derivative of$V$to find whether each of these extrema is a minimum (positive value) or a maximum (negative value).<br>
...
def _(mo):
    mo.md(r"""
    Discarding the solution$b=\dfrac{a}{2}$(where$V=0$, which is a minimum),$b=\dfrac{a}{6}$results in the maximum volume.<br>
...
def _(mo):
    ...
    Gradient descent is a first-order iterative optimization algorithm for finding the minimum of a function ([Wikipedia](https://en.wikipedia.org/wiki/Gradient_descent)).<br>
...
def _(mo):
    ...
    From https://en.wikipedia.org/wiki/Gradient_descent:<br>
...
def _(mo):
    ...
    \end{array}$Where$pcsa_i$is the physiological cross-sectional area of muscle$i$and$M_{max,i}$is the maximum torque muscle$i$can produce.<br>
...
def _(mo):
    ...
    2. Regarding the distribution problem for the elbow muscles presented in this text:<br>
        a. Test different initial values for the optimization.<br>
        b. Test other values for the elbow angle where the results are likely to change.

    3. In an experiment to estimate forces of the elbow flexors, through inverse dynamics it was found an elbow flexor moment of 10 Nm.<br>
    Consider the following data for maximum force (F0), moment arm (r), and pcsa (A) of the brachialis, brachioradialis, and biceps brachii muscles: F0 (N): 1000, 250, 700; r (cm): 2, 5, 4; A (cm$^2$): 33, 8, 23, respectively (data from Robertson et al. (2013)).<br>
        a. Use static optimization to estimate the muscle forces.<br>
        b. Test the robustness of the results using different initial values for the muscle forces.<br>
