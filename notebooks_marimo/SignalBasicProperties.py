...
def _(mo):
    ...
    A function is said to be periodic with period$T$if$x(t+T) = x(t)$for all values of$t$, that is, the function repeats itself after a period. Important properties of this definition are that the sum of periodic functions and a constant times a periodic function are also periodic functions. In particular, this means that a periodic function also repeats itself after$2T,\: 3T, \dots$.<br>
...
def _(mo):
    ...
    With respect to what is being measured, data can be classified as signal or noise.<br>
    <a href="https://en.wikipedia.org/wiki/Signal_(electrical_engineering)">Signal</a> is what you want of the phenomenon and you believe this signal truly represents the phenomenon being measured (or modeled) and [noise](https://en.wikipedia.org/wiki/Noise) is what you don't want in a measurement because, in principle, it has no significant role to the understanding of the observed phenomenon.<br>
    As much as this argument is tautological, the distinction between noise and signal depends on what you think is relevant about the observed phenomenon. Experimental data are contaminated by noise and it is rarely possible to completely distinguish signal from noise.<br>
...
def _(mo):
    ...
    8. Consider the continuous signal represented by the function$x(t) = 2\sin(8\pi t)$.<br>
     a) Which would be the Nyquist frequency ($f_N$) of this signal?<br>
    ...
    For example, the following code generates a sinusoid (signal) plus a random noise:<br>
    ```python
