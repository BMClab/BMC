...
def _(mo):
    mo.md(r"""
    For better numerical accuracy (and also to distinguish values in the whole quadrant), the inverse function of <span class="notranslate">$\tan$</span> is preferred.<br>
...
def _(mo):
    ...
    In certain cases, we have access to the 3D coordinates of markers but we just care for the angle between segments in the plane defined by these segments (but if there is considerable movement in different planes, this simple 2D angle might give unexpected results).<br>
...
def _(mo):
    ...
    Consider a particle rotating around a point at a fixed distance `r` (circular motion), as the particle moves along the circle, it travels an arc of length `s`.<br>
...
def _(mo):
    ...
    1. A gymnast performs giant circles around the horizontal bar (with the official dimensions for  Artistic Gymnastics) at a constant rate of one circle every 2 s and consider that his center of mass is 1 m distant from the bar. At the lowest point (exactly beneath the bigh bar), the gymnast releases the bar, moves forward, and lands standing on the ground.<br>
     a) Calculate the angular and lineat velocity of the gymnast's center of mass at the point of release.<br>
     b) Calculate the horizontal distance travelled by the gymnast's center of mass.

    2. With the data from Table A1 of Winter (2009) and the convention for the sagital joint angles of the lower limb:<br>
     a. Calculate and plot the angles of the foot, leg, and thigh segments.<br>
     b. Calculate and plot the angles of the ankle, knee, and hip joint.<br>
     c. Calculate and plot the velocities and accelerations for the angles calculated in B.
     d. Compare the ankle angle using the two different conventions described by Winter (2009), that is, defining the foot segment with the MT5 or the TOE marker.<br>
