...
def _(mo):
    ...
    So far we assumed $r_M = 0.033$ m, fixed. In reality the moment arms of the knee extensors are not constant: they change with the knee angle, because the patella and the tendon wrap over the femoral condyles as the joint rotates. We can import measured data from [OpenSim](https://simtk.org/home/opensim). Here is the data for the vastus intermedius moment arm.

    We read it straight from a URL rather than from a local file, for the same reason the muscle model is defined inside this notebook: it keeps the notebook runnable anywhere, including in the browser.
    """)
    return
...
@app.cell
def _(np):
    _url = "https://raw.githubusercontent.com/BMClab/BMC/master/data/vas_int_r.mot"
    try:  # in the browser (marimo.app runs on Pyodide) urllib has no sockets
        from pyodide.http import open_url

        _source = open_url(_url)
    except ImportError:
        from urllib.request import urlopen

        _source = urlopen(_url)

    # columns of the OpenSim .mot file: time, knee_angle_r (deg), vas_int_r (m)
    vas_int_r = np.loadtxt(_source, skiprows=7)
    print("Shape of vas_int_r:", vas_int_r.shape)
    print(
        "Knee angle range: %.1f to %.1f deg"
        % (vas_int_r[:, 1].min(), vas_int_r[:, 1].max())
    )
    print(
        "Moment arm range: %.4f to %.4f m"
        % (vas_int_r[:, 2].min(), vas_int_r[:, 2].max())
    )
    return (vas_int_r,)
...
def _(mo):
    mo.md(r"""
    The moment arm varies by roughly a factor of two over the range of knee angles — hardly a constant. But we need to be able to calculate it at *any* angle the solver asks for, not only at the 100 tabulated values. Let's fit a spline to the data, which returns a function we can call at any angle:
    """)
    return


@app.cell
def _(interpolate, vas_int_r):
...
def _(S, plt, vas_int_r):
    _fig, _ax = plt.subplots(nrows=1, ncols=1, figsize=(7, 4))
    _ax.plot(vas_int_r[:, 1], vas_int_r[:, 2], label="Raw data")
    _ax.plot(vas_int_r[:, 1], S(vas_int_r[:, 1]), "r--", label="Spline")
    _ax.set_xlabel(r"Knee angle $(^o)$")
    _ax.set_ylabel("Moment arm (m)")
    plt.legend(loc="best")
    plt.tight_layout()
    plt.show()
    return
...
def _(mo):
    mo.md(r"""
    It works as expected. We just have to handle the fact that the two angle conventions are different, and this is worth getting right:

    - In our model, $\theta$ is measured in **radians** from the horizontal, so $\theta = \pi/2$ means the knee is flexed at $90^o$ and $\theta = \pi$ means the knee is fully extended.
    - In the OpenSim data, `knee_angle_r` is in **degrees**, and $0^o$ means full extension, with flexion counted as negative (the data run from $-120^o$ to $+10^o$).

    So the conversion is: turn $\theta$ from radians into degrees, then subtract $180^o$.

    $\text{knee angle} = \dfrac{180}{\pi}\theta - 180$

    A useful sanity check: at $\theta = \pi/2$ this gives $-90^o$, and the spline there returns $0.0331$ m — almost exactly the constant $0.033$ m that Nigg and Herzog used. The two models agree at the starting posture and diverge from there.

    We also clip the angle to the range that was actually measured, so that the spline is never asked to extrapolate beyond its data.
    """)
    return


@app.cell
def _(S, np, vas_int_r):
    _angle_min = vas_int_r[:, 1].min()
    _angle_max = vas_int_r[:, 1].max()

    def rm_var(theta):
        """Vastus intermedius moment arm [m] as a function of theta [rad]."""

        knee_angle = np.clip(theta * 180 / np.pi - 180, _angle_min, _angle_max)

        return float(S(knee_angle))

    print("Moment arm at theta = 90 deg: %.4f m" % rm_var(np.pi / 2))
    print("Moment arm at theta = 135 deg: %.4f m" % rm_var(3 * np.pi / 4))
    return (rm_var,)
...
def _(mo):
    mo.md(r"""
    Now we solve exactly the same system of ODEs, changing only the moment-arm function we hand to it.

    **Before you run the next cells**, predict: the moment arm *grows* as the knee extends. A larger moment arm means more joint moment for the same muscle force — but it also means, from $L_{MT} = L_{MT0} - r_M(\theta - \pi/2)$, that the muscle has to shorten faster for the same rotation. Faster shortening means less force. Which effect do you think wins? Will the knee extend faster or slower than with the constant moment arm?
    """)
    return


@app.cell
def _(odeint, onelink_eq, rm_var, state0, time):
    data_var = odeint(onelink_eq, state0, time, args=(rm_var,))
    return (data_var,)


@app.cell
def _(data_var, ms, rm_var, sim_plot, time):
    data2_var = sim_plot(time, data_var, ms, rm_var)
    return (data2_var,)


@app.cell
def _(data2_var, ms):
    axs_lm2 = ms.lm_plot(data2_var)
    return
...
def _(mo):
    mo.md(r"""
    **Challenge 4.** Compare the two knee-angle plots directly. With the measured moment arm the knee reaches roughly $168^o$ after $0.12$ s, against about $181^o$ with the constant one — the movement is *slower*, even though the moment arm is larger over most of the range. Go back to the two competing effects in the prediction above and explain which one won, using the muscle force panel as evidence.

    **Challenge 5.** With the constant moment arm the simulated knee passes $180^o$, that is, it hyperextends past a straight leg. Real knees do not do this. What is missing from our model? Name at least two structures or effects that would stop the movement in a real leg.
    """)
    return
