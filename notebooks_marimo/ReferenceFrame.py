...
def _(mo):
    ...
    The concept of reference frame in Biomechanics and motor control is very important and central to the understanding of human motion. For example, do we see, plan and control the movement of our hand with respect to reference frames within our body or in the environment we move? Or a combination of both?<br>
    ...
    For three-dimensional motion analysis in Biomechanics, we may use several different references frames for convenience and refer to them as global, laboratory, local, anatomical, or technical reference frames or coordinate systems (we will study this later).<br>
...
def _(mo):
    ...
    One can see that the versors of the basis shown in the figure above have the following coordinates in the Cartesian coordinate system:<br>
    <br>
    ...
    Using the notation described in the figure above, the position vector$\overrightarrow{\mathbf{r}}$(or the point$\overrightarrow{\mathbf{P}}$) can be expressed as:<br>
    <br>
...
def _(mo):
    ...
    Let's now define a basis using a common method in motion analysis (employing the cross product):<br>
...
def _(mo):
    mo.md(r"""
    Due to rounding errors ([see here](https://en.wikipedia.org/wiki/Round-off_error)) and to the imprecision of the computation with decimal floating-point numbers ([see here](http://docs.python.org/2/tutorial/floatingpoint.html)), the norms are not exactly equal to 1.<br>
...
def _(mo):
    mo.md(r"""
    Let's plot the coordinate system and the basis using the custom Python function `CCS.py`.<br>
...
def _(mo):
    mo.md(r"""
    The first versor is in the <span class="notranslate">$\overrightarrow{\mathbf{a}}$</span> direction (or in the direction of any of the other vectors):<br>
    <br>
...
def _(mo):
    mo.md(r"""
    The second versor, orthogonal to$\hat{e}_a$, can be found considering we can express vector$\overrightarrow{\mathbf{b}}$in terms of the$\hat{e}_a$direction as:<br>
    <br>
...
def _(mo):
    mo.md(r"""
    The third versor, orthogonal to$\{\hat{e}_a, \hat{e}_b\}$, can be found expressing the vector$\overrightarrow{\mathbf{C}}$in terms of$\hat{e}_a$and$\hat{e}_b$directions as:<br>
    <br>
