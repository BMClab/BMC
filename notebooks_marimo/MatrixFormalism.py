...
def _():
    ...
    for _i in range(1, len(t)):
        ...
        M = np.array(
            [
                [m1 * l1**2 / 3 + m2 * l1**2, m2 * l1 * l2 * np.cos(thetaDiff) / 2],
                [m2 * l1 * l2 * np.cos(thetaDiff) / 2, m2 * l2**2 / 3],
            ]
        )
        C = np.array(
            [
                [-m2 * l1 * l2 * np.sin(thetaDiff) * state[1, _i - 1] ** 2 / 2],
                [m2 * l1 * l2 * np.sin(thetaDiff) * state[0, _i - 1] ** 2 / 2],
            ]
        )
        G = np.array(
            [
                [
                    -m1 * g * l1 / 2 * np.sin(state[2, _i - 1])
                    - m2 * g * l2 * np.sin(state[2, _i - 1])
                ],
                [-m2 * g * l2 / 2 * np.sin(state[3, _i - 1])],
            ]
        )
        Q = np.array([[M1 - M12], [M12]])
        dstatedt = np.vstack(
            (np.linalg.inv(M) @ (C + G + Q), state[0, [_i - 1]], state[1, [_i - 1]])
        )
...
def _(l1, l2, np, plt, state):
    ...
    for _i in range(len(state[2, 0::step])):
        plt.plot(
            [0, l1 * np.sin(state[2, _i * step])],
            [0, -l1 * np.cos(state[2, _i * step])],
        )
        plt.plot(
            [
                l1 * np.sin(state[2, _i * step]),
                l1 * np.sin(state[2, _i * step]) + l2 * np.sin(state[3, _i * step]),
            ],
            [
                -l1 * np.cos(state[2, _i * step]),
                -l1 * np.cos(state[2, _i * step]) - l2 * np.cos(state[3, _i * step]),
            ],
        )
    plt.show()
    return
