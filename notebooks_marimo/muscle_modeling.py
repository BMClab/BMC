...
@app.function
def fpelm_M03(lm=1, lmopt=1, fm0=1, lmslack=1, wp=0.6):
...
def _():
    def fvce_T03_symb():
        ...
        a, flce, fm, af, fmlen, vmmax = symbols(
            "a, flce, fm, af, fmlen, vmmax", positive=True
        )
...
def _(mo):
    mo.md(r"""
    We will solve the equation for $a$ by numerical integration using an explicit Runge-Kutta method of order (4)5 due to Dormand and Prince (a.k.a. `ode45` in Matlab):
    """)
    return
...
def _(np):
    def actdyn_ode45(
        fun, t0=0, t1=1, a0=0, t_act=0.015, t_deact=0.050, u_max=1, u_min=0.01
    ):
        ...
        _t_eval = np.arange(t0, t1 + 0.005, 0.01)
        _solution = solve_ivp(
            _rhs,
            (t0, t1),
            [a0],
        ...
        if not _solution.success:
            raise RuntimeError(_solution.message)

        _excitation = np.array([excitation(_t, u_max, u_min) for _t in _solution.t])
        _activation = np.maximum(_solution.y[0], u_min)
        return np.column_stack((_solution.t, _excitation, _activation))
...
def _(act1_M03, act1_T03, act2_M03, act2_T03, act3_M03, act4_M03, plt):
    _fig, _axs = plt.subplots(
        nrows=2, ncols=2, sharex=True, sharey=True, figsize=(8, 5)
    )
...
def _(mo):
    ...
    - Yamaguchi GT (2001) *Dynamic Modeling of Musculoskeletal Motion: A Vectorized Approach*. Chapter 2: An Introduction to Modeling Muscle and Tendon. See local file `courses/refs/Yamaguchi_ch2.pdf`.
