...
def angle(height, velocity):
    ...
    if velocity != 0:
        ang = (
            math.atan(velocity / math.sqrt(velocity**2 + 2 * g * height))
            * 180
            / math.pi
        )
...
def _():
    ...
    print(
        "The optimum angle for a jump with h=%.1f m and v=%.1f m/s is %.1f degrees"
        % (height, velocity, ang)
    )
    return
...
def _():
    ...
    sns.set_context(
        "notebook", font_scale=1.2, rc={"lines.linewidth": 2, "lines.markersize": 10}
    )
    return (cm,)
...
def _(cm, np, plt):
    ...
    v, h = np.meshgrid(np.arange(5, 10.25, 0.25), np.arange(0, 1.05, 0.05))
