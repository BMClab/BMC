...
def _(mo):
    ...
    Next, we will study two simple problems of multibody systems in the context of biomechanics which we can handle well using the Newton-Euler approach.<br>
    First a planar one-link system (which is not a multibody), which can represent the movement of one limb of the body or the entire body as a single inverted pendulum.<br>
...
def _(mo):
    ...
    \end{array}$Note that$\times$represents the cross product, not matrix multiplication. Then, both in symbolic or numeric manipulation we would use the cross product function to perform part of the calculations.<br>
    There are different computational tools that can be used for the formulation of the equations of motion. For instance, Sympy has a module, [Classical Mechanics](http://docs.sympy.org/dev/modules/physics/mechanics/), and see [this list](http://real.uwaterloo.ca/~mbody/#Software) for other software.<br>
...
def _(mo):
    mo.md(r"""
    As an exercise, let's go back to the Newton-Euler equation for the sum of torques around the center of mass where the torques due to the joint reaction forces are explicit.<br>
...
def _(mo):
    ...
    Another way to deduce the velocity and acceleration of a point at the rotating link is to use the [Jacobian matrix](http://en.wikipedia.org/wiki/Jacobian_matrix_and_determinant) (see [Kinematic chain](http://nbviewer.ipython.org/github/demotu/BMC/blob/master/notebooks/KinematicChain.ipynb)).<br>
    Remember that in the context of kinematic chains, the Jacobian relates changes in the joint space to changes in the Cartesian space. The Jacobian is a matrix of all first-order partial derivatives of the linear position vector of the endpoint with respect to the angular position vector.<br>
...
def _(mo):
    ...
    We could simulate the movement of this one-link system for a typical human movement to understand the magnitude of these physical quantities.<br>
...
def _(mo):
    mo.md(r"""
    There is an elegant form to display the equations for the torques using generalized coordinates,$q=[\alpha_1, \alpha_2]^T$and grouping the terms proportional to common quantities in matrices, see for example, Craig (2005, page 180), Pandy (2001), and Zatsiorsky (2002, page 383):$\tau \quad=\quad M(q)\ddot{q} + C(q,\dot{q}) + G(q) + E(q,\dot{q})$Where, for this two-link system:<br>
    -$\tau$is a matrix (2x1) of joint torques;<br>
    -$M$is the mass or inertia matrix (2x2);<br>
    -$\ddot{q}$is a matrix (2x1) of angular accelerations;<br>
    -$C$is a matrix (2x1) of [centipetal](http://en.wikipedia.org/wiki/Centripetal_force) and [Coriolis](http://en.wikipedia.org/wiki/Coriolis_effect) torques;<br>
    -$G$is a matrix (2x1) of  gravitational torques;<br>
...
def _(mo):
    ...
    Where$r_{nm}$is the moment arm about joint$n$of the muscle$m$.<br>
...
def _(mo):
    ...
    5. Consider the double pendulum moving in the horizontal plane and with no external force. Find out the type of movement and which torque terms are changed when:<br>
