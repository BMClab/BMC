...
def _(mo):
    ...
    *Python 2.x is legacy, Python 3.x is the present and future of the language*.<br>
    [Python is transitioning from version 2 to 3](https://wiki.python.org/moin/Python2orPython3) and version 3 breaks backwards compatibility for some functions and because of that part of the comunnity hasn't made this transition yet. The third-party Python libraries for scientific computing have to be rewritten for Python 3; most of them are already compatible, but it seems the community is not ready yet.<br>
    For instance, the Anaconda distribution ships with both versions but defaults to Python 2.x (download Anaconda3 for default Python 3) and the number of packages for Python 3.x is still smaller than for Python 2.x [[1](http://docs.continuum.io/anaconda/pkgs.html), [2](https://python3wos.appspot.com/)].<br>
...
def _(mo):
    ...
    **The easy way**<br>
    The easiest way to get Python and the most popular packages for scientific programming is to install them with a Python distribution such as [Anaconda](https://www.continuum.io/anaconda-overview).<br>
    ...
    **Anaconda Installation Instructions**<br>
    Go to the [*Anaconda* website](https://www.anaconda.com/download/) and download the appropriate version for your computer (but download Anaconda3! for Python 3.x). The file is big (about 500 MB). [From their website](https://docs.anaconda.com/anaconda/install/):<br>
    **Linux Install**<br>
    In your terminal window type and follow the instructions:<br>
    ```
    bash Anaconda3-4.4.0-Linux-x86_64.sh
    ```
    **OS X Install**<br>
    For the graphical installer, double-click the downloaded .pkg file and follow the instructions<br>
    For the command-line installer, in your terminal window type and follow the instructions:<br>
    ```
