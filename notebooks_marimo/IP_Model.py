...
def _(mo):
    mo.md(r"""
    The equations of motion for each FBD of the feet and rest-of-body segments at the sagittal plane ($xy$plane) can be expressed in the form of the Newton-Euler equations.<br>
    <br>
...
def _(mo):
    ...
    \end{array}$Where$I_a$is the moment of inertia of the whole body around the ankle joint.<br>
    During the standing still posture, the$GRF$horizontal component is typically much smaller than the$GRF$vertical component and the torque of the former can be neglected. In addition, the magnitude of the$GRF$vertical component is approximately constant and equal to the body weight.<br>
    ...
    Finally, the last equation can be expressed in the following form:$COG_v - COP \approx \frac{I_a}{mgh} \ddot{COG}_v$Or simply:$COG_v - COP \approx k \, \ddot{COG}_v$Where$k = I_a/(mgh)$.<br>
    If the human body is represented as a rigid bar, its moment of inertia will be approximately equal to$1.33mh^2$, so$k \approx 1.33h/g$.<br>
...
def _(mo):
    ...
    These two last equations express a very simple relation between body segment parameters,$COG_v$,$COP$, body acceleration, and horizontal force.<br>
    Solely based on these equations it is possible to predict some interesting relations among the variables in these equations, which have been experimentally observed:<br>
     -$COG_v-COP$is positively correlated with the horizontal ground reaction force in the anterior-posterior direction (Winter et al. 1998; Morasso and Schieppati 1999; Zatsiorsky and Duarte 2000);<br>
    ...
    Obviously, the most straightforward prediction of the single inverted pendulum model concerning the **kinematics of the segments** of the human body would be that we should observe merely the motion at the ankle joint and nothing in the other joints. This prediction has not been observed (see for example, Pinter et al. 2008 and Gunther et al. 2009). During standing still, we seem to use all our joints and this is task dependent.<br>
...
def _(mo):
    ...
    Based on the inverted pendulum model, it's possible to estimate the$COG_v$displacement from the$COP$displacement after some mathematical manipulation we show next.<br>
    Back to the relation between$COG_v$and$COP$displacements, it has the form:$y(t) - x(t) = k\,\ddot{y}(t)$Where$y(t)$stands for the$COG_v$signal and$x(t)$for the$COP$signal, which are functions of time, and$k = I_a/(mgh)$.<br>
    ...
    <br>
    Let's apply the Fourier transform to the differential equation with$COG_v$and$COP$:$Y(j\omega) - X(j\omega) = -k\,\omega^2Y(j\omega)$Where we defined$y(t) \Leftrightarrow Y(j\omega)$and$x(t) \Leftrightarrow X(j\omega)$as the Fourier pairs,$j$is the imaginary unit, and$\omega$is the angular frequency,$2\pi f$.<br>
    The reason why we use the Fourier transform is because we started with a second order differential equation and ended with the simple algebraic equation above. Rearranging the equation above:$\frac{Y(j\omega)}{X(j\omega)} = \frac{\omega_0^2}{\omega_0^2 + \omega^2}$Where$\omega_0 = 1/\sqrt{k}$.<br>
    If we imagine a system where the$COP$is the input and the$COG_v$the output, the right side of the equation above is known as the <a href="http://en.wikipedia.org/wiki/Transfer_function">transfer function</a> of such system, the ratio between the output and the input.<br>
