...
class OrdinaryDifferentialEquationTests(unittest.TestCase):
    ...
    def test_notebook_has_conversational_tutorial_challenges_and_links(self):
        ...
        self.assertIn("only have access to the car's acceleration", source)
        self.assertIn("smartphone accelerometer", source)
        self.assertIn("\\left\\{", source)
        self.assertIn("x_{i+1} \\approx x_i + v_i\\Delta t", source)
        self.assertIn("v_{i+1} \\approx v_i + a_i\\Delta t", source)
        self.assertIn("coupled equations", source)
        self.assertIn("| 2 | 20 | 0.5 | 30 | 550 |", source)
        self.assertIn("| 3 | 30 | -- | 35 | ? |", source)
        self.assertIn("Use the row at \\(t=20\\ \\mathrm{s}\\)", source)
        self.assertIn("calculate the new position at \\(t=30\\ \\mathrm{s}\\)", source)
        self.assertIn("two coupled equations", source)
        self.assertIn("double numerical integration", source)
        self.assertIn("come back to the car", source)
        self.assertIn("play the role of position and velocity", source)
        self.assertIn("acceleration-like rate of change", source)
