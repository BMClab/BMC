...
def pdf_norm_plot(m=0, s=1, fig=None, ax=None):
    ...
    ax.text(-1.9*s+m, .03*ymax, '13.6%',  color='w')
    ax.text(1.1*s+m, .03*ymax, '13.6%', color='w')
    ax.text(-.9*s+m, .08*ymax, '34.1%', color='w')
    ax.text(.1*s+m, .08*ymax, '34.1%', color='w')
    ax.set_xlim(-1*s, 1*s)
    plt.locator_params(axis = 'y', nbins = 5)
    ax.set_xticks(np.linspace(m-4*s, m+4*s, 9))
    xtl = [r'-%d$\sigma$' %i for i in range(4, 0, -1)]
    xtl.extend([r'$\mu$'])
    xtl.extend([r'+%d$\sigma$' %i for i in range(1, 5, 1)])
